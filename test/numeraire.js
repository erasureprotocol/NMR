...
contract('Numeraire', function(accounts) {
  before(function () {
    var nmr = Numeraire.deployed();
    return nmr.mint(accounts[1], 10000000000, { from: accounts[0] });
  });
  ...
  it("should not mint more than disbursement", function() {
    ...
    return nmr.mint(accounts[1], 76000000000000, { from: accounts[0] }).then(() => {
  ...
  it('should reduce disbursement when minting', () => {
    ...
    return nmr.disbursement.call(accounts[0]).then(last_disbursement => {
      return nmr.mint(accounts[2], 1, { from: accounts[0] })
  ...
  it("should reset disbursement once per week", function() {
    ...
    return nmr.disbursement.call(accounts[0]).then(disbursement => {
      return nmr.mint(accounts[1], disbursement.toNumber() / 2, { from: accounts[0] }).then(() => {
        return web3.evm.increaseTime(7 * 25 * 60 * 60).then(() => {
          return nmr.mint(accounts[1], 2400000000000, { from: accounts[0] }).then(() => {
            return nmr.disbursement.call(accounts[0]).then(disbursement => {
  ...
  it("should not mint more than 10,000,000", function() {
    ...
    return nmr.mint(accounts[1], 510000000000000, { from: accounts[0] }).then(() => {
  ...
  it("should send NMR correctly", function() {
    ...
    return nmr.mint(accounts[1], amount, { from: accounts[0] }).then(function () {
      return nmr.balanceOf.call(account_one).then(function(balance) {
        account_one_starting_balance = balance.toNumber();
        return nmr.balanceOf.call(account_two);
      }).then(function(balance) {
        account_two_starting_balance = balance.toNumber();
        return nmr.transfer(account_two, amount, { from: account_one });
      }).then(function() {
  ...
  it('should stake NMR', () => {
    ...
    return nmr.balanceOf.call(accounts[0])
      .then(() => nmr.balanceOf.call(accounts[1]).then((balance) => {
        return nmr.stake(accounts[1], 500, { from: accounts[0] }).then(() => {
  ...
  it('should release stake', function() {
    var nmr = Numeraire.deployed();
    return nmr.stake(accounts[1], 500, { from: accounts[0] }).then(tx_id => {
      var txn = web3.eth.getTransaction(tx_id);
      var block = web3.eth.getBlock(txn.blockNumber);
      return nmr.releaseStake(accounts[1], block.timestamp, { from: accounts[0] });
    });
  });
  ...
  it('should destroy stake', function() {
    ...
    return nmr.stake(accounts[1], 500, { from: accounts[0] }).then(tx_id => {
      var txn = web3.eth.getTransaction(tx_id);
      var block = web3.eth.getBlock(txn.blockNumber);
      return nmr.destroyStake(accounts[1], 1, { from: accounts[0] });
    });
  });
});
