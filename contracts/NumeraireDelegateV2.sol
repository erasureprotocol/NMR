...
contract NumeraireDelegate is StoppableShareable, NumeraireShared {
    ...
    function createTournament(uint256 _tournamentID) onlyManyOwners(sha3(msg.data)) returns (bool ok) {
        var tournament = tournaments[_tournamentID];
    ...
    function createRound(uint256 _tournamentID, uint256 _roundID, uint256 _endTime, uint256 _resolutionTime) onlyManyOwners(sha3(msg.data)) returns (bool ok) {
        var tournament = tournaments[_tournamentID];
        var round = tournament.rounds[_roundID];
